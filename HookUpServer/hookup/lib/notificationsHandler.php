...
function activateNotificationHandler()
{
    ...
    $command = PHP_COMMAND . " " . ROOT_DIR . "/lib/notificationsHandler.php '" . serialize(
            $params
        ) . "' > /dev/null &";
    system($command);
}
...
function handlePostRequest($data)
{
    ...
    if ($requestType == 'message') {
        ...
        if ($targetType == 'group') {
            ...
            $url = $db_url . "/_design/app/_view/find_lastwatching_group_by_group_id?key=" . urlencode(
                    '"' . $toGroup . '"'
                );
            $userListResultJSON = getRequest($url, true);
            $userListResult = json_decode($userListResultJSON, true);
...
function newGroupNotification($toUserId, $fromUserId, $toGroupId, $type)
{
    ...
    if (isset($userActivitySummary)) {
        ...
        $result = putRequest(
            $db_url . $userActivitySummary["_id"] . "/?rev=" . $userActivitySummary["_rev"],
            $json,
            true
        );

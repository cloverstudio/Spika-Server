...
function sendGCM($deviceToken, $message, $fromUser="", $groupId="", $type="user", $fromUserName="") {
	...
	$fields = array(
		    ...
		    'data'              => array( "message" => $message, "fromUser" => $fromUser,"fromUserName"=>$fromUserName, "type" => $type, "groupId" => $groupId ),
		    );
...
function sendGroupMessageNotification($userDataFrom,$toUser,$toGroupData,$message){
	...
	if(!empty($userDataTo['ios_push_token'])){
		sendAPNProd($userDataTo['ios_push_token'],$message,"","",array('from_user' => $userDataFrom['value']['_id'],'to_group' => $toGroupData['_id'],'type'=>'group'));
		sendAPNDev($userDataTo['ios_push_token'],$message,"","",array('from_user' => $userDataFrom['value']['_id'],'to_group' => $toGroupData['_id'],'type'=>'group'));
		
	}
	
	if(!empty($userDataTo['android_push_token'])){
		SendGCM($userDataTo['android_push_token'],$message, $userDataFrom['value']['_id'], $toGroupData['_id'], 'group',$userDataFrom['value']['name']);
	}
...
function sendDirectMessageNotification($fromUser,$toUser,$message){
	...
	if(!empty($userDataTo['value']['android_push_token'])){
		SendGCM($userDataTo['value']['android_push_token'],$message, $fromUser,"","user",$userDataFrom['value']['name']);
	}	
}
...
function sendPushToQueue($serviceProvidor,$token,$payload){
	...
	$fields_string = "";
	foreach($postParam as $key=>$value) { $fields_string .= $key.'='.$value.'&'; }
	rtrim($fields_string, '&');
	...
	function putRequest($url, $reqBody, $return = false){
			...
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, true); 
			curl_setopt($curl, CURLOPT_HTTPHEADER, array("Content-Type: application/json", 'Content-Length: ' . strlen($reqBody)));	
			curl_setopt($curl, CURLOPT_POSTFIELDS, $reqBody);
			curl_setopt($curl, CURLOPT_HEADER, 1);

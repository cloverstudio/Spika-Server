<?php
	...
	if(isset($_POST['token']) && isset($_POST['payload']) && isset($_POST['service_provider'])){
		...
		}else if($serviceProvider == SERVICE_PROVIDOR_APN_DEV){
			$result = sendAPNDev($token,$payload);
		}else if($serviceProvider == SERVICE_PROVIDOR_GCM){
							...
							<div class="box-content nopadding">
								...
								<form action="" method="POST" id="form1" class='form-horizontal form-bordered'>
									<div class="control-group">
										...
										<div class="controls">
											<input type="text" name="message" id="message" placeholder="" class="input-xxlarge" value="<?php echo $message ?>">
										</div>
									</div>
									<div class="control-group">
										<label for="textfield" class="control-label">token</label>
										<div class="controls">
											<input type="text" name="token" id="token" placeholder="" class="input-xxlarge" value="<?php echo $token ?>">
										</div>
									</div>
									<div class="control-group">
										<label class="control-label">Service Provider<small>Please select one of them</small></label>
										<div class="controls">
											<label class='radio'>
												<input <?php if($serviceProvider == 1) echo 'checked=checked';?> type="radio" name="service_provider" id="service_provider1" value="<?php echo SERVICE_PROVIDOR_APN_PROD ?>"> APN production
											</label>
											<label class='radio'>
												<input <?php if($serviceProvider == 2) echo 'checked=checked';?> type="radio" name="service_provider" id="service_provider2" value="<?php echo SERVICE_PROVIDOR_APN_DEV ?>"> APN development
											</label>
											<label class='radio'>
												<input <?php if($serviceProvider == 3) echo 'checked=checked';?> type="radio" name="service_provider" id="service_provider3" value="<?php echo SERVICE_PROVIDOR_GCM?>"> GCM
											</label>
										</div>
									</div>
									<div class="control-group">
										<label for="textarea" class="control-label">Payload<small>Please make sure JSON is formatted properly.</small></label>
										<div class="controls">
											<textarea name="payload" id="payload" rows="5" class="input-block-level"><?php echo $payload ?></textarea>
										</div>
									</div>
